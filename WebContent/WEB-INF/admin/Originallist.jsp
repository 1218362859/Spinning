...
<SCRIPT type="text/javascript">
	function find() {
		if (true) {
			...
			document.forms.form2.action = " ${pageContext.request.contextPath}/user/findOriginallist.action";
			...
			<div class="am-btn-toolbars am-btn-toolbar am-kg am-cf">
				<ul>
					...
					<form id="form2" method="post">
						<li style="width: 300px;"><input type="text" name="nickname"  value="${nickname}"
							style="width: 100%" class="am-form-field am-input-sm am-input-xm"
						placeholder="可模糊查询 用户昵称" /></li>
			...
			<form class="am-form am-g">
					...
					<thead>
						<tr class="am-success">
							...
							<th class="table-type">出生年月</th>
									<th class="table-type">原始数据</th>
										<th class="table-type">原始数据创建时间</th>
					...
					<tbody>
						...
						<c:forEach items="${queryByPage.list}" var="user">
							<tr>
								...
								<td>${ user.height}</td>
								<td>${ user.birthday}</td>
								<td>${ user.bluetooth_data}</td>
									<td>${ user.create_time}</td>
								<td><div class="am-btn-toolbar">
										<div class="am-btn-group am-btn-group-xs">
											<a type="button" onClick="return del()"
												href="${pageContext.request.contextPath }/user/deleteOriginal.action?id=${user.id}"
				...
				<ul class="am-pagination am-fr">
					...
					<c:if test="${queryByPage.pageNum!=1 }">
						<li><a
							href="${pageContext.request.contextPath }/user/findOriginallist..action?nickname=${nickname }&pageNo=1&pageSize=10">首页</a></li>
						<li><a
							href="${pageContext.request.contextPath }/user/findOriginallist..action?nickname=${nickname }&pageNo=${ queryByPage.pageNum-1}&pageSize=10">上一页</a></li>
					</c:if>

					<c:if test="${queryByPage.pageNum!= queryByPage.lastPage}">
						<li><a
							href="${pageContext.request.contextPath }/user/findOriginallist..action?nickname=${nickname }&pageNo=${ queryByPage.pageNum+1}&pageSize=10">下一页</a></li>
						<li><a
							href="${pageContext.request.contextPath }/user/findOriginallist..action?nickname=${nickname }&pageNo=${ queryByPage.lastPage}&pageSize=10">尾页</a></li>
					</c:if>
				</ul>
